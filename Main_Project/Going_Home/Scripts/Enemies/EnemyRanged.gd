...
func rangedAttack():
	...
	var projectile = get_node("Projectile").get_child(0)
...
func getPlayDist():
	...
	var playX : float = get_tree().get_root().get_node("Player").get_position().x
	var playY : float = get_tree().get_root().get_node("Player").get_position().y
